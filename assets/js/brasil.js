...
document.addEventListener('DOMContentLoaded', async () => {
  const weatherSP = await fetch(`${proxy}https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=Sao Paulo&lang=pt`)
  ...
  const weatherRJ = await await fetch(`${proxy}https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=Rio de janeiro&lang=pt`)
  ...
  const weatherMG = await fetch(`${proxy}https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=Minas Gerais&lang=pt`)
