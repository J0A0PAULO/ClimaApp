...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const weatherRJ = await fetch(`${proxy}${encodeURIComponent(`https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=Rio de Janeiro&lang=pt`)}`)
  ...
  const weatherMG = await fetch(`${proxy}${encodeURIComponent(`https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=Minas Gerais&lang=pt`)}`)
